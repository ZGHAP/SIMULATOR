...
OUT_DIR = Path(__file__).parent / "output/sim"
...
def main():
    ...
    # Determine tick size
    instrument = df["instrument"].iloc[0] if "instrument" in df.columns else path.stem.split("_")[0]
    tick = args.tick_size or (0.1 if "SC" in instrument.upper() else 1.0)
    print(f"Instrument: {instrument}  tick_size: {tick}")

    df["pnl"] = (df["exit_price"] - df["entry_price"]) * df["side"].map({"long": 1, "short": -1}) / tick
    ...
    out = Path(__file__).parent / f"output/{instrument.lower()}_pnl_analysis.png"
